...
GENDER, SIGN, PHOTO, LOCATION, BIO = range(5)
...
def gender(update, _):
    ...
    reply_keyboard = [['Овен', 'Телец', 'Близнецы', 'Рак', 'Лев','Дева', 'Весы',
            'Скорпион', 'Козерог','Водолей','Рыбы', 'Стрелец']]
    markup_key = ReplyKeyboardMarkup(reply_keyboard, one_time_keyboard=True)
    update.message.reply_text(
        'Здорово. А какой у тебя знак зодиака?\n '
        'отправь /skip, если стесняешься.',
        reply_markup=markup_key,)
